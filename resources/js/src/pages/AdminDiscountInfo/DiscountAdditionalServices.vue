<template>
  <b-container fluid="lg">
    ...
    <b-row class="mt">
      <b-col cols="3">
        <b-form-group>
          <label class="_label_input">Service / Product Type <span class="_require">*</span></label>
          <multiselect
            :show-labels="false"
            :allow-empty="false"
            deselect-label=""
            :class="{
              'form-group--error': $v.discountParams.service_type.$error,
            }"
            v-model.trim="$v.discountParams.service_type.$model"
            :options="listTypeOther"
            placeholder="Select one"
            track-by="id"
            label="reference_value_text"
            @input="handleGetListServiceByType"
          ></multiselect>
          <div class="error" v-if="!$v.discountParams.service_type.required && $v.discountParams.service_type.$error">Field is required</div>
        </b-form-group>
      </b-col>
      <b-col cols="3">
        <b-form-group>
          ...
          <multiselect
            ...
            :class="{
              'form-group--error': $v.discountParams.service.$error,
            }"
            v-model.trim="$v.discountParams.service.$model"
            :options="listServiceByType"
            track-by="id"
            label="service_name"
            placeholder="Select one"
          ></multiselect>
          <div class="error" v-if="!$v.discountParams.service.required && $v.discountParams.service.$error">Field is required</div>
        </b-form-group>
      </b-col>
    </b-row>
    <b-row class="mt">
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      listServiceByType: (state) => state.booking.listServiceByType,
    }),
  },
  methods: {
    ...mapActions({
      ...
      getListServiceByType: "booking/getListServiceByType",
    }),
    linkClass(idx) {
    ...
    onSave() {
      ...
      prms.other_id = prms.service.id;
      prms.type = prms.service_type.id;
      prms.amount_type = prms.amount_type.id;
      prms.discount_service = "Additional Services";
    ...
    handleGetListServiceByType(val) {
      if (val) {
        this.getListServiceByType(val.id)
          .then((response) => {
    ...
    handleDiscountParams: function () {
      if (!isEmpty(this.discountDetail)) {
        ...
        this.discountParams.service = this.discountDetail.service;
        this.discountParams.amount = this.discountDetail.amount;
        this.discountParams.remarks = this.discountDetail.remarks;
        this.discountParams.service_type = this.discountDetail.type;
      }
    },
  },
  watch: {
