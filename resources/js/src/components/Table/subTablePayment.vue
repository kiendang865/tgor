<template>
  ...
  >
    ...
    >
      ...
      <td></td>
      <td></td>
      <td class="pl-3">{{totalDetail ? handleDefaultTotal(totalDetail) : 0 | formatMoney}}</td>
    </template>
  </b-table>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    handleDefaultTotal(data){
      let amt_paid = 0
      this.tableItems.forEach((value, index) => {
        amt_paid = this.unFormatter(amt_paid) + this.unFormatter(value.amount)
      })
      //check total Payable
      let total = this.unFormatter(data.amount) - this.unFormatter(data.discount);
      let newDiscount = parseFloat(this.gstNumber) * total;
      let result = (total + newDiscount) - this.minusAmountPaid();
      // this.checkPayment = result > 0 ? false : true;
      this.$emit('checkPayment2',result > 0 ? false : true)
