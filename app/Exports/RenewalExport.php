...
class RenewalExport implements FromView
{
    ...
    public function view(): View
    {
        ...
        ->whereHas('invoice.invoice_line_item.sale_agreement_line_item.booking_line_item', function($query){
            $query->whereNotNull('renewal_from_id');
        })
            ...
            $query->with(['invoice_line_item' => function($q){
                $q->with(['sale_agreement_line_item' => function($_q){
                    $_q->with(['booking_line_item' => function($_q1){
                        $_q1->with('niche', 'information', 'referral', 'booking_type');
                    }]);
                }]);
            }]);
        }])
        ...
        return view('exports.renewal', compact('collections'));
    }
}
